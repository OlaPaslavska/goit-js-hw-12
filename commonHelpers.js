...
</li>`).join("");c.gallery.innerHTML("beforeend",t)}function b(){c.loader.classList.remove("hidden")}function m(){c.loader.classList.add("hidden")}function L(){c.loadMoreBtn.classList.remove("hidden")}function g(){c.loadMoreBtn.classList.add("hidden")}function S(r,t){r>=t?(g(),t&&n.info({title:"The end!",message:"We're sorry, but you've reached the end of search results."})):L()}const a={form:document.querySelector(".form"),input:document.querySelector(".input"),button:document.querySelector("button"),gallery:document.querySelector(".gallery"),loader:document.querySelector(".loader"),loadMoreBtn:document.querySelector(".load-more-btn")};let d="",l=1,p=15,y=1;const B=new M(".gallery a",{captions:!0,captionsData:"alt",captionDelay:250});form.addEventListener("submit",async r=>{if(r.preventDefault(),d=input.value.trim(),l=1,g(),d===""){a.gallery.innerHTML=" ",n.warning({title:"Warning",message:"Please, enter the query.",backgroundColor:"#ef4040",layout:2,position:"topRight",displayMode:"once"}),a.form.reset();return}b(),a.gallery.innerHTML=" ";try{const t=await f(d,l,p);if(y=Math.ceil(t.totalHits/p),y===0){n.error({message:"Sorry, there are no images matching your search query. Please try again!",layout:2,displayMode:"once",backgroundColor:"#ef4040",progressBarColor:"#B51B1B",position:"topRight"}),a.form.reset(),m();return}m(),a.form.reset(),h(t.hits),B.refresh(),L()}catch(t){n.error({title:"Error",message:`${t}`,layout:2,displayMode:"once",backgroundColor:"#ef4040",progressBarColor:"#B51B1B",position:"topRight"})}});a.loadMoreBtn.addEventListener("click",async()=>{g(),b();try{l++;const r=await f(d,l,p);r.hits.length!==0&&(h(r.hits),B.refresh(),m()),S(l,y),skipOldElement()}catch(r){a.gallery.innerHTML=" ",n.error({title:"Error",message:`${r}`,layout:2,displayMode:"once",backgroundColor:"#ef4040",progressBarColor:"#B51B1B",position:"topRight"})}});
