...
</li>`).join("");u.gallery.insertAdjacentHTML("beforeend",t)}function b(){u.loader.classList.remove("hidden")}function c(){u.loader.classList.add("hidden")}function L(){u.loadMoreBtn.classList.remove("hidden")}function g(){u.loadMoreBtn.classList.add("hidden")}const a={form:document.querySelector(".form"),input:document.querySelector(".input"),button:document.querySelector("button"),gallery:document.querySelector(".gallery"),loader:document.querySelector(".loader"),loadMoreBtn:document.querySelector(".load-more-btn")};let d="",i=1,y=15,m=1;const q=new B(".gallery a",{captions:!0,captionsData:"alt",captionDelay:250});a.form.addEventListener("submit",async r=>{if(r.preventDefault(),d=a.input.value.trim(),i=1,d===""){a.gallery.innerHTML=" ",n.warning({title:"Warning",message:"Please, enter the query.",backgroundColor:"#ef4040",layout:2,position:"topRight",displayMode:"once"}),a.form.reset();return}b(),a.gallery.innerHTML=" ",g();try{const t=await f(d,i,y);if(m=Math.ceil(t.totalHits/y),t.hits.length===0){n.error({message:"Sorry, there are no images matching your search query. Please try again!",layout:2,displayMode:"once",backgroundColor:"#ef4040",progressBarColor:"#B51B1B",position:"topRight"}),a.form.reset(),c();return}h(t.hits),q.refresh(),i<m?L():g(),c(),a.form.reset()}catch(t){n.error({title:"Error",message:`${t}`,layout:2,displayMode:"once",backgroundColor:"#ef4040",progressBarColor:"#B51B1B",position:"topRight"})}});a.loadMoreBtn.addEventListener("click",async()=>{g(),b(),i++;try{const r=await f(d,i,y);r.hits.length==0||i>=m?(n.info({title:"Info",message:"We're sorry, but you've reached the end of search results.",position:"topCenter",timeout:5e3}),c()):(h(r.hits),q.refresh()),i<m&&L(),c()}catch(r){n.error({title:"Error",message:`Error loading images: ${r}`,position:"topRight",timeout:5e3}),c()}});
